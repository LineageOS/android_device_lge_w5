...
static int hw_acc_set_enable_inputev(uint32_t enable)
{
    int err = 0;

    char path[128] = "";

    PDEBUG("acc enable: %d", enable);

    sprintf(path, "%s/input%d/%s",
                    SYSFS_PATH_INPUT_DEV, g_input_dev_num_a, SYSFS_NODE_NAME_BMA_ENABLE);
...
static void hw_init_a_settings(struct sensor_hw *hw) {
     ...
     */
    struct sensor_hw_a *hw_a = CONTAINER_OF(hw,
...
static int hw_enable_a(struct sensor_hw *hw, int enable) {
    ...
    err |= hw_acc_set_enable_inputev(enable);
